...
import sys, os, pdb, subprocess, struct
# import cv2
import numpy as np
...
def extract_flows(vid_dir, frame_dir, gpu, redo=False):

  class_list = sorted(os.listdir(os.path.join(vid_dir, 'train')))
  print("Classes =", class_list)
  splits = ['train', 'test', 'val']
  for splt in splits:
    vid_dir_split = os.path.join(vid_dir, splt)
    for ic,cls in enumerate(class_list):
        vlist = sorted(os.listdir(os.path.join(vid_dir_split, cls)))
        print("")
        print(ic+1, len(class_list), cls, len(vlist))
        print("")
        for v in vlist:
            print(v)
            outdir = os.path.join(frame_dir, splt, cls, v)
...
if __name__ == '__main__':
  ...
  gpu       = int(sys.argv[3])
  extract_flows(vid_dir, frame_dir, gpu, redo=True)